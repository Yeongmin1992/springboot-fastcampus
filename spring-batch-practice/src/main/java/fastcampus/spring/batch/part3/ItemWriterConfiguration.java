...
//                .next(this.jdbcBatchItemWriterStep())
    ...
    private ItemWriter<Person> csvFileItemWriter() throws Exception {
        ...
        FlatFileItemWriter<Person> itemWriter = new FlatFileItemWriterBuilder<Person>()
                ...
                .encoding("UTF-8")
                .resource(new FileSystemResource("output/test-output.csv")) // 파일 생성성
                .lineAggregator(lineAggregator)
                .headerCallback(writer -> writer.write("id,이름,나이,거주지"))  // 맨위에 헤더 삽입 -> 기존파일 덮어씀
                .footerCallback(writer -> writer.write("-----------------\n"))   // 맨 밑에 풋터 삽입 -> 기존파일 덮어씀
                .append(true) // 이거 설정하고 풋터 맨 뒤에 개행문자를 넣어줘야 행 바꿔서 밑에 데이터 추가 됨 (덮어쓰기가 아니라 추가)
                .build();
